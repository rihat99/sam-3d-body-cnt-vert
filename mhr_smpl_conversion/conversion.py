...
class Conversion:
    ...
    def __init__(
        ...
        if smpl_model.num_vertices == _NUM_VERTICES_SMPL:
            self.smpl_model_type = "smpl"
            self._hand_pose_dim = 0
        elif smpl_model.num_vertices == _NUM_VERTICES_SMPLX:
            self.smpl_model_type = "smplx"
            self._hand_pose_dim = 6 if getattr(smpl_model, "use_pca", False) else 45
        else:
            raise ValueError(
                f"Unsupported SMPL model type! Expected {_NUM_VERTICES_SMPL} or {_NUM_VERTICES_SMPLX} vertices, got {smpl_model.num_vertices}"
            )
    ...
    # ── MHR model compatibility helpers ───────────────────────────────────

    @cached_property
    def _mhr_character(self):
        """Return the MHR character object regardless of whether the model is
        a full ``mhr.mhr.MHR`` instance or a TorchScript ``RecursiveScriptModule``.

        - Full MHR: exposes ``.character`` (pymomentum Character object)
        - TorchScript: exposes ``.character_torch`` (scripted module)
        """
        if hasattr(self._mhr_model, "character"):
            return self._mhr_model.character
        return self._mhr_model.character_torch

    @cached_property
    def _mhr_rest_vertices(self) -> torch.Tensor:
        """Rest-pose MHR vertices as a tensor on the conversion device."""
        ch = self._mhr_character
        # Full MHR character uses .mesh.vertices; TorchScript uses .mesh.rest_vertices
        if hasattr(ch.mesh, "vertices"):
            verts = ch.mesh.vertices
        else:
            verts = ch.mesh.rest_vertices
        if isinstance(verts, torch.Tensor):
            return verts.to(self._DEVICE)
        return torch.from_numpy(np.asarray(verts)).float().to(self._DEVICE)

    @cached_property
    def _mhr_mesh_faces(self) -> torch.Tensor:
        """MHR face connectivity as a long tensor on the conversion device."""
        faces = self._mhr_character.mesh.faces
        if isinstance(faces, torch.Tensor):
            return faces.long().to(self._DEVICE)
        return torch.from_numpy(np.asarray(faces, dtype=np.int64)).long().to(self._DEVICE)

    @cached_property
    def _mhr_param_names(self) -> list[str] | None:
        """Parameter names from the MHR character's parameter_transform, or None
        if the model is TorchScript (which does not expose names as strings)."""
        pt = self._mhr_character.parameter_transform
        if hasattr(pt, "names"):
            return pt.names
        return None
    ...
    @cached_property
    def _smpl_faces_tensor(self) -> torch.Tensor:
        """SMPL triangle connectivity cached on the conversion device."""
        faces = self._smpl_model.faces
        if isinstance(faces, torch.Tensor):
            return faces.cpu().long().to(self._DEVICE)
        return (
    ...
    @cached_property
    def _mhr_faces_tensor(self) -> torch.Tensor:
        """MHR triangle connectivity cached on the conversion device."""
        return torch.from_numpy(load_mhr_faces()).long().to(self._DEVICE)

    @cached_property
    def _smpl2mhr_contact_mapping(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Cached (triangle_ids, baryc_coords) tensors for SMPL → MHR contact mapping."""
        tri_ids, baryc = load_surface_mapping("smpl2mhr", self.smpl_model_type)
        return (
            torch.from_numpy(tri_ids).long().to(self._DEVICE),
            torch.from_numpy(baryc).float().to(self._DEVICE),
        )

    @cached_property
    def _mhr2smpl_contact_mapping(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Cached (triangle_ids, baryc_coords) tensors for MHR → SMPL contact mapping."""
        tri_ids, baryc = load_surface_mapping("mhr2smpl", self.smpl_model_type)
        return (
            torch.from_numpy(tri_ids).long().to(self._DEVICE),
            torch.from_numpy(baryc).float().to(self._DEVICE),
        )
    ...
    def _interpolate_contacts(
        self,
        contacts: torch.Tensor | np.ndarray,
        triangle_ids: torch.Tensor,
        baryc_coords: torch.Tensor,
        source_faces: torch.Tensor,
        threshold: float = 0.5,
    ) -> torch.Tensor:
        """Barycentric interpolation of per-vertex binary contact labels.

        Interpolates contact labels from the source mesh topology to the target
        topology using precomputed barycentric weights, then binarises the
        result with *threshold*.

        Args:
            contacts: Binary (or float) contact labels on the source mesh.
                Shape ``[V_src]`` (single frame) or ``[B, V_src]`` (batch).
            triangle_ids: ``[V_tgt]`` — index of the source triangle for each
                target vertex.
            baryc_coords: ``[V_tgt, 3]`` — barycentric weights.
            source_faces: ``[F, 3]`` — source mesh face connectivity.
            threshold: Binarisation threshold (default 0.5).

        Returns:
            Long tensor of shape ``[B, V_tgt]`` with values in ``{0, 1}``.
            Single-frame inputs (1-D) are returned with the batch dim kept for
            consistency; callers may squeeze if needed.
        """
        if isinstance(contacts, np.ndarray):
            contacts = torch.from_numpy(contacts)
        contacts = contacts.float().to(self._DEVICE)
        if contacts.dim() == 1:
            contacts = contacts.unsqueeze(0)

        # [V_tgt, 3]: three source-vertex indices per target vertex
        tri_vertex_ids = source_faces[triangle_ids]      # [V_tgt, 3]
        tri_vals = contacts[:, tri_vertex_ids]           # [B, V_tgt, 3]
        interp = (tri_vals * baryc_coords).sum(dim=-1)   # [B, V_tgt]
        return (interp >= threshold).long()
    ...
    def convert_smpl2mhr(
        ...
        contacts: torch.Tensor | np.ndarray | None = None,
        ...
        Returns:
            ConversionResult containing:
                ...
                - result_contacts: Long tensor [B, 18439] of MHR contact labels (if contacts is not None)
        ...
        result_contacts = None
        if contacts is not None:
            tri_ids, baryc = self._smpl2mhr_contact_mapping
            result_contacts = self._interpolate_contacts(
                contacts, tri_ids, baryc, self._smpl_faces_tensor, contact_threshold
            )
    ...
    def convert_mhr2smpl(
        ...
        Returns:
            ConversionResult containing:
                ...
                - result_errors: Numpy array of fitting errors for each frame (if return_fitting_errors=True)
                - result_contacts: Long tensor [B, 6890] of SMPL contact labels (if contacts is not None)
        """
        if batch_size is not None:
            self._batch_size = batch_size
        ...
        result_contacts = None
        if contacts is not None:
            tri_ids, baryc = self._mhr2smpl_contact_mapping
            result_contacts = self._interpolate_contacts(
                contacts, tri_ids, baryc, self._mhr_faces_tensor, contact_threshold
            )
    ...
    def _compute_target_vertices(
        ...
        if direction == "smpl2mhr":
            ...
            source_faces = self._mhr_character.mesh.faces
        else:
            raise ValueError(
        ...
        if isinstance(source_faces, torch.Tensor):
            source_faces_tensor = source_faces.long().to(self._DEVICE)
        else:
    ...
    def _s2m_get_mhr_param_mask(
        ...
        When parameter names are unavailable (TorchScript model), falls back to
        all-True masks for "params_for_no_hand" and all-False masks for
        "pose_params_for_head" (i.e. no masking / no head-only mode).
        """
        pt = self._mhr_character.parameter_transform
        num_pose_param = int(pt.pose_parameters.sum() - 6)
        num_scale_params = int(pt.scaling_parameters.sum())

        # TorchScript model does not expose parameter names as strings
        lbs_parameter_names = self._mhr_param_names
        if lbs_parameter_names is not None:
            lbs_parameter_names = lbs_parameter_names[
    ...
    def _get_identity_parameter_mask(self) -> torch.Tensor:
        ...
        """
        scale_mask = self._mhr_character.parameter_transform.scaling_parameters
    ...
    def _s2m_fit_mhr_using_pymomentum(
        ...
        if single_identity:
            ...
            selected_frames = self._select_frames_for_identity_estimation(
                target_vertices,
                self._mhr_rest_vertices.cpu().numpy(),
                self._mhr_mesh_faces.cpu().numpy(),
            )
    ...
    def _mhr_para2mesh(
        ...
        """
        faces = self._mhr_mesh_faces.cpu().numpy()
        meshes = []
        mhr_vertices = []
        ...
        for batch_start in tqdm(range(0, num_samples, self._batch_size)):
            ...
            with torch.no_grad():
                smpl_vertices.append(
                    self._smpl_forward(batch_parameters).detach().cpu().numpy()
                )
        ...
        for i, frame_idx in enumerate(failure_indices):
            ...
            with torch.no_grad():
                ...
                sequence_vertices = self._compute_target_vertices(
                    self._smpl_forward(interpolated_params), "smpl2mhr"
                )  # [steps+1, V_mhr, 3]
    ...
    def _smpl_edges(self) -> torch.Tensor:
        ...
        smpl_template_mesh = trimesh.Trimesh(
            self._smpl_model.vertices_template.cpu().numpy(),
    ...
    def _smpl_forward(self, params: dict[str, torch.Tensor]) -> torch.Tensor:
        """Run SMPL forward pass from classic axis-angle parameters.

        Converts the classic dict (``betas``, ``body_pose``, ``global_orient``,
        ``transl``) to the better_human quaternion representation via
        ``from_classic``, then runs the model forward pass.

        Args:
            params: Dict with keys ``betas`` [B, K], ``body_pose`` [B, 69],
                ``global_orient`` [B, 3], and optionally ``transl`` [B, 3].

        Returns:
            Vertex positions [B, V, 3].
        """
        betas = params["betas"]
        B = betas.shape[0]
        transl = params.get("transl", torch.zeros(B, 3, device=betas.device))
        q = self._smpl_model.from_classic(
            betas=betas,
            body_pose=params["body_pose"],
            translation=transl,
            global_orient=params["global_orient"],
        )
        return self._smpl_model(betas=betas, q=q).vertices
