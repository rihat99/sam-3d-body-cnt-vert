...
class ConversionResult:
    ...
    result_contacts: torch.Tensor | None = None
    """Binary contact labels on the *target* topology.  Shape ``[B, V_tgt]`` (long)."""
...
@lru_cache
def load_mhr_faces() -> np.ndarray:
    """Load MHR triangle face connectivity from the bundled PLY asset."""
    tmp_mesh = trimesh.load(MHR_FACE_MASK_FILE, process=False)
    return np.asarray(tmp_mesh.faces, dtype=np.int64)


def evaluate_model_fitting_error(
    model: MHR | BetterHumanSMPL,
    ...
    with torch.no_grad():
        ...
        for batch_start in range(0, num_frames, batch_size):
            ...
            if model_type == "mhr":
                ...
                q = model.from_classic(
                    betas=betas,
                    body_pose=batch_params["body_pose"],
                    translation=batch_params.get(
                        "transl",
                        torch.zeros(betas.shape[0], 3, device=betas.device),
                    ),
                    global_orient=batch_params["global_orient"],
                )
                batch_vertices = model(betas=betas, q=q).vertices
            else:
                raise ValueError(
