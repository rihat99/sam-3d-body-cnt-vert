...
class PyTorchSMPLFitting(BasePyTorchFitting):
    ...
    def _select_frames_for_identity_estimation(
        ...
        return super()._select_frames_for_identity_estimation(
            target_vertices=target_vertices,
            template_vertices=self._smpl_model.vertices_template,
    ...
    def _evaluate_conversion_error(
        ...
        for batch_start in range(0, num_frames, self._batch_size):
            ...
            batch_params = {
                k: torch.stack(
                    [fitting_parameter_results[k][i] for i in batch_indices], dim=0
                ).to(self._device)
                for k in ("betas", "body_pose", "global_orient", "transl")
                if k in fitting_parameter_results
            }
            batch_vertices = self._smpl_forward(batch_params)
    ...
    def _optimize_one_batch(
        ...
        smpl_verts = self._smpl_forward(batched_smpl_parameters)
        smpl_edges = (
